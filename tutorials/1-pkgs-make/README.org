...
* Following along with code
  ...
  application provides the shell script which is called =example-shell=:
...
*** Example application code
    ...
    reflection: ~writeShellScriptBin~, ~hello~, and ~example-shell-lib~.
...
* Pinning Nixpkgs
  ...
  When you pin to a specific version of Pkgs-make, it's defaults to pinning
  ...
  Getting the revision of Nixpkgs is simple. That's just the commit ID from Git
  for the project. To get the SHA-256 of the download, we could make a similar
  call to =nix-prefetch-url= as we made for pinning Pkgs-make earlier:
