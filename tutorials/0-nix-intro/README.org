...
* Org-mode Setup                                                   :noexport:
  ...
  #+BEGIN_SRC shell :results output silent :noweb yes
    ...
    rmdir --recursive --force /tmp/some.d
  #+END_SRC
...
* Motivation for Nix
  ...
  computers of the same platform architecture, we get an extremely high
  ...
  Nix is not without its problems.  Nix could use more documentation and tools
  to ease adoption. Also though Nix has a healthy community of contributors, it
  still takes a lot of work to curate all of Nixpkgs, so you may occasionally
  find yourself writing your own Nix expressions, or contributing back to the
  Nix ecosystem. Fortunately, once you know what you're doing, many Nix
  expressions are compact and easy to modify or override.
...
* Exploring a Fresh Nix Installation
  ...
  - depending on you OS, files under =/etc/system.d/= (Linux's systemd) or
    =/Library/LaunchDaemons= (MacOS's launchd) to start up a “nix-daemon”
    service used by Nix commands.
...
** String interpolation
   ...
   useful to have string interpolation support. This is done with the following
   syntax:
...
** Attribute sets
   ...
   #+BEGIN_SRC shell :results output :exports both
     nix eval '(builtins.typeOf { a = 1; b = 2; })'
     nix eval '({ a = 1; b = 2; }.b)'
   #+END_SRC
   ...
   #+BEGIN_SRC shell :results output :exports both
     nix eval '({ a = b; b = 2; }.a)' 2>&1 || true
   #+END_SRC

   #+RESULTS:
   : error: undefined variable 'b' at (string):1:8
   ...
   #+BEGIN_SRC shell :results output :exports both
     nix eval '(({ a }: a + 2 ) { a = 3; b = 4; })' 2>&1 || true
   #+END_SRC

   #+RESULTS:
   : error: anonymous function at (string):1:3 called with unexpected argument 'b', at (string):1:2

   If we want to relax the destructuring to accept sets with other attributes
   we can use a “...” form:

   #+BEGIN_SRC shell :results output :exports both
     nix eval '(({ a, ...}: a + 2 ) { a = 3; b = 4; })'
   #+END_SRC
...
** Paths
   ...
   Because the Nix language was designed for building packages, file paths come
   up frequently in Nix expressions. Nix conveniently has a /path/ type, which
   is indicated by identifiers with at least one slash:
   ...
   path if found. For example, we can consider =/tmp= as a path prefix when
   looking up =some.d/path= to find =/tmp/some.d/path=:
