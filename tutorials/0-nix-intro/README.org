...
** Strings
   ...
   Notice how the first token after the “''” establishes a left-margin for the
   multi-line string.  Beyond strings, Nix does not have syntactically
...
** Attribute sets
   ...
   unpack the argument as a convenience with the following syntax:
   ...
   We can still bind the argument to a name if we want using a “@” form, and if
   we want to accept sets with other attributes we can use a “...” form:
   ...
   in locally bound values as attributes, which comes up a lot in Nix:
   ...
   Using the ~inherit~ key word, we accomplish the same thing while simplifying
   the ~a = a~ to:
...
* Nixpkgs and building
  ...
  that the derivation itself is also saved into =/nix/store= as a “*.drv” file,
  and when the application is actually built, it is put in another output path,
  also in =/nix/store=.
...
* Calling =nix=
  ...
  With this setting of ~NIX_PATH~, the set =nix= would choose an attribute from
  would be:
...
* Inspecting dependencies
  ...
  shouldn't break if we do something like upgrade to our host operating system.
...
* Developing with =nix-shell=
  ...
  Without this switch, =nix-env= looks for file called =shell.nix= to import in
  ...
  In our derivation we can include as build dependencies not only things like
