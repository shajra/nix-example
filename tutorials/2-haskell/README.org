...
** Basic build
   ...
   =cabal2nix= to generate a Nix expression from a Cabal file on our behalf.
...
** Modifying the Haskell build
   ...
   Some derivations have multiple outputs, and whose derivations can be
   accessed by attributes on the derivation. For instance, to get to the
   generated Haddocks for our ~example-haskell-lib~ library, we can reference
   the ~example-haskell-lib.doc~ attribute:
...
** Editor tags files
   ...
   projects fluidly using [[ctags][Ctags/Etags]]. For Haskell, this tutorial's Nix shell
   ...
   In Vim, you can now use ~Ctrl-]~ and ~Ctrl-t~ to jump to declarations, even
   in the source code for third-party projects and the standard/base libraries.
...
** Haskell Stack
   ...
   **WARNING**: This project uses Stack's [[stack-nix][built-in support for Nix integration]].
   ...
   are congruent to those used by Nix.
